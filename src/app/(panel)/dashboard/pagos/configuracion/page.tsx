...
import {
  ...
  Wallet, CheckCircle2, Loader2, Check, Settings,
} from 'lucide-react'
    ...
    <motion.div variants={stagger} initial="hidden" animate="visible" className="mx-auto max-w-3xl">
      ...
      <motion.div variants={fadeUp} className="mb-6">
        {wallet ? (
          ...
          >
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#334155] aspect-[1.586/1] max-h-[220px] shadow-lg transition-shadow group-hover:shadow-xl">
                          ...
                          {config?.campos.slice(0, 3).map((campo) => (
                            <div key={campo.key} className="flex items-baseline gap-2">
                              ...
                              <span className="text-xs font-semibold text-white/90 truncate">
                                {metodo.datos[campo.key] ? (
                                  metodo.datos[campo.key].length > 28
                                    ? metodo.datos[campo.key].slice(0, 28) + '...'
                                    : metodo.datos[campo.key]
                                ) : '—'}
                              </span>
                            </div>
