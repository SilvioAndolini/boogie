...
func (s *ResenaService) Crear(ctx context.Context, userID string, input *CrearResenaInput) (string, string, error) {
	...
	return resenaID, reserva.PropiedadID, nil
}
...
func (s *ResenaService) Responder(ctx context.Context, input *ResponderInput) (string, error) {
	...
	if err := s.repo.SetRespuesta(ctx, input.ResenaID, input.Respuesta); err != nil {
		return "", err
	}

	return resena.PropiedadID, nil
}
